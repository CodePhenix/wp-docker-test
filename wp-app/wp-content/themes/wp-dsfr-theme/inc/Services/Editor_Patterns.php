...
class Editor_Patterns implements Service {
	...
	public function register_categories(): void {
		...
		/**
		 * usage : 'common' => [ 'label' => __( 'Common', 'wp-dsfr-theme' ) ]
		 */
		$pattern_categories = [
			'alert'     => [ 'label' => __( 'Alerte', 'wp-dsfr-theme' ) ],
			'badge'     => [ 'label' => __( 'Badge', 'wp-dsfr-theme' ) ],
			'btn'       => [ 'label' => __( 'Bouton', 'wp-dsfr-theme' ) ],
			'callout'   => [ 'label' => __( 'Mise en avant', 'wp-dsfr-theme' ) ],
			'hero'      => [ 'label' => __( 'Hero', 'wp-dsfr-theme' ) ],
			'highlight' => [ 'label' => __( 'Mise en exergue', 'wp-dsfr-theme' ) ],
			'icon'      => [ 'label' => __( 'Icône', 'wp-dsfr-theme' ) ],
			'link'      => [ 'label' => __( 'Lien', 'wp-dsfr-theme' ) ],
			'table'     => [ 'label' => __( 'Tableau', 'wp-dsfr-theme' ) ],
			'misc'      => [ 'label' => __( 'Divers', 'wp-dsfr-theme' ) ],
		];
				...
				foreach ( $files as $file ) {
					...
					if ( empty( $pattern_data['slug'] ) ) {
						_doing_it_wrong(
							...
							/* translators: %s: file name. */
								esc_html__( 'Could not register file "%s" as a block pattern ("Slug" field missing)', 'wp-dsfr-theme' ),
								esc_html( $file )
							),
							'6.0.0'
						);
					...
					if ( ! preg_match( '/^[A-z0-9\/_-]+$/', $pattern_data['slug'] ) ) {
						_doing_it_wrong(
							...
							/* translators: %1s: file name; %2s: slug value found. */
								esc_html__( 'Could not register file "%1$s" as a block pattern (invalid slug "%2$s")', 'wp-dsfr-theme' ),
								esc_html( $file ),
					...
					if ( ! $pattern_data['title'] ) {
						_doing_it_wrong(
							...
							sprintf(
							/* translators: %1s: file name; %2s: slug value found. */
								esc_html__( 'Could not register file "%s" as a block pattern ("Title" field missing)', 'wp-dsfr-theme' ),
								esc_html( $file )
							),
							'6.0.0'
						);
